...
def compute_prediction_errors(
    x_e: float, x_hat_e: float, x_i: float, x_hat_i: float
) -> tuple[float, float]:
...
def compute_precision(
    sigma: float, pi_min: float = 0.01, pi_max: float = 100.0, eps_stab: float = 1e-8
) -> float:
...
```python
def compute_metabolic_cost(S: float, B_prev: float, c1: float = 0.2, c2: float = 0.1) -> float:
...
def compute_information_value(
    z_e: float, z_i_eff: float, v1: float = 0.3, v2: float = 0.2
) -> float:
...
```python
def compute_ignition_soft(S: float, theta: float, tau_sigma: float = 0.5) -> float:
...
class LiquidStateMachine:
    ...
    def train_readout(self, X: np.ndarray, y: np.ndarray, alpha: float = 1e-6):
...
def compute_landauer_cost(
    S: float, eps: float, k_b: float = 1.38e-23, T_env: float = 310.0, kappa_meta: float = 1.0
) -> float:
...
class StabilityAnalyzer:
    ...
    def analyze_bifurcation(self, param_name: str, param_range: np.ndarray) -> dict:
